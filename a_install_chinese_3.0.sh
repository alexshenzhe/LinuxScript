...
tomcatInstall(){
	...
	if cat /etc/profile | grep CATALINA_HOME_$system_name >/dev/null
		...
		sed -i '$a##########'$(echo $system_name)' tomcat##########' /etc/profile
		sed -i '$aTOMCAT_HOME_'$(echo $system_name)'='$(echo $tomcat_file_path)'' /etc/profile
		sed -i '$aCATALINA_HOME_'$(echo $system_name)'='$(echo $tomcat_file_path)'' /etc/profile
		sed -i '$aCATALINA_BASE_'$(echo $system_name)'='$(echo $tomcat_file_path)'' /etc/profile
		sed -i '$aexport TOMCAT_HOME_'$(echo $system_name)'  CATALINA_HOME_'$(echo $system_name)'  CATALINA_BASE_'$(echo $system_name)'' /etc/profile
		sed -i '$a##########'$(echo $system_name)' tomcat##########' /etc/profile
...
supermapInstall(){
	...
	if [ ! -d $all_packages_path/$DependentFileName ]; then
		if [ ! -f $all_packages_path/$DependentPackageName ]; then
			...
			# 安装依赖包
			rpm -ivh --force --nodeps $all_packages_path/$DependentFileName/*.rpm
...
addUser(){
	...
	if [ $? -ne 0 ]  
		then  
	    adduser $user_name 
	    passwd $user_name --stdin &>/dev/null
		chmod -v u+w /etc/sudoers
		sed -i '$a'$(echo $user_name)'  ALL=(ALL)       ALL' /etc/sudoers
		chmod -v u-w /etc/sudoers 
...
deleteUser(){
	...
	if [ $? -ne 0 ]  
		...
		case $val in
			Y|y)  userdel -r $user_name
				  if [ $? -eq 0  ]; then
					chmod -v u+w /etc/sudoers
				  	sed -i -e "/$user_name/d" /etc/sudoers
				  	chmod -v u-w /etc/sudoers
...
JDKInstall(){
	...
	if cat /etc/profile | grep JAVA_HOME >/dev/null
		...
		# 修改系统环境变量
		sed -i '$a########### JAVA Environment #############' /etc/profile
		sed -i '$aexport JAVA_HOME='$(echo $JDKFilePath)'' /etc/profile
		sed -i '$aexport JRE_HOME=$JAVA_HOME/jre' /etc/profile
		sed -i '$aexport CLASSPATH=./:$JAVA_HOME/lib:$JAVA_HOME/jre/lib' /etc/profile
	...
	if cat /home/yuantiaotech/.bashrc | grep JAVA_HOME >/dev/null
		...
		# 修改用户环境变量
		sed -i '$a########### JAVA Environment #############' /home/yuantiaotech/.bashrc
		sed -i '$aexport JAVA_HOME='$(echo $JDKFilePath)'' /home/yuantiaotech/.bashrc
		sed -i '$aexport JRE_HOME=$JAVA_HOME/jre' /home/yuantiaotech/.bashrc
		sed -i '$aexport CLASSPATH=./:$JAVA_HOME/lib:$JAVA_HOME/jre/lib' /home/yuantiaotech/.bashrc
		sed -i '$aexport PATH=$PATH:$JAVA_HOME/bin' /home/yuantiaotech/.bashrc
		# 生效配置
		source /home/yuantiaotech/.bashrc
		echo "JAVA_HOME=$JDKFilePath" >> /etc/environment
	...
	ln -s $JDKFilePath/ /usr/java/
	mv /usr/java/$JDKFileName /usr/java/jdk1.7
...
SupervisorInstall(){
	...
	# 拷贝启动脚本到/etc/init.d/
	cp $all_packages_path/supervisord /etc/init.d/
	# 检测java路径配置
	...
	if cat /etc/init.d/supervisord | grep $javaPath >/dev/null
		...
		sed -i '/export PATH=/d'  /etc/init.d/supervisord
	    sed -i '3a export PATH='$(echo $javaPath)'' /etc/init.d/supervisord
	    if [ $? -eq 0 ]; then
			let count+=1
...
MySQLUninstall(){
	...
	if [ ! -n "$oldMySQL" ]; then
		...
		yum -y remove MySQL-server-5.6.36-1.el6.x86_64
		yum -y remove MySQL-client-5.6.36-1.el6.x86_64
		yum -y remove MySQL-embedded-5.6.36-1.el6.x86_64
		yum -y remove MySQL-shared-5.6.36-1.el6.x86_64
		yum -y remove MySQL-test-5.6.36-1.el6.x86_64
		yum -y remove MySQL-devel-5.6.36-1.el6.x86_64
		yum -y remove MySQL-shared-compat-5.6.36-1.el6.x86_64
...
MySQLInstall(){
	...
	sleep 1
	rpm -ivh --nodeps --force $all_packages_path/MySQL-client-5.6.36-1.el6.x86_64.rpm
	rpm -ivh --nodeps --force $all_packages_path/MySQL-devel-5.6.36-1.el6.x86_64.rpm
	rpm -ivh --nodeps --force $all_packages_path/MySQL-embedded-5.6.36-1.el6.x86_64.rpm
	rpm -ivh --nodeps --force $all_packages_path/MySQL-server-5.6.36-1.el6.x86_64.rpm
	rpm -ivh --nodeps --force $all_packages_path/MySQL-shared-5.6.36-1.el6.x86_64.rpm
	rpm -ivh --nodeps --force $all_packages_path/MySQL-shared-compat-5.6.36-1.el6.x86_64.rpm
	rpm -ivh --nodeps --force $all_packages_path/MySQL-test-5.6.36-1.el6.x86_64.rpm
	if [ $? -eq 0 ]; then
		let count+=1
	...
	if [ ! -f /etc/my.cnf ]; then
		...
		sed -i '$adatadir=/var/lib/mysql' /etc/my.cnf
		sed -i '$asocket=/var/lib/mysql/mysql.sock' /etc/my.cnf
		sed -i '$auser=mysql' /etc/my.cnf
		sed -i '$aport=3306' /etc/my.cnf
		sed -i '$alower_case_table_names=1' /etc/my.cnf
		sed -i '$acharacter-set-server=utf8' /etc/my.cnf
		sed -i '$asymbolic-links=0' /etc/my.cnf
		sed -i '$askip-name-resolve' /etc/my.cnf
		sed -i '$anet_write_timeout=600' /etc/my.cnf
		sed -i '$await_timeout=31536000' /etc/my.cnf
		sed -i '$ainteractive_timeout=31536000' /etc/my.cnf
		sed -i '$acollation-server=utf8_general_ci' /etc/my.cnf
		sed -i '$askip-external-locking' /etc/my.cnf
		sed -i '$akey_buffer=512M' /etc/my.cnf
		sed -i '$amax_allowed_packet=16M' /etc/my.cnf
		sed -i '$athread_stack=192K' /etc/my.cnf
		sed -i '$athread_cache_size=8' /etc/my.cnf
		sed -i '$aread_rnd_buffer_size=2560M' /etc/my.cnf
		sed -i '$asort_buffer_size=2560M' /etc/my.cnf
		sed -i '$ajoin_buffer_size=2560M' /etc/my.cnf
		sed -i '$ainnodb_buffer_pool_size=6G' /etc/my.cnf
		sed -i '$aread_buffer_size=2560M' /etc/my.cnf
		sed -i '$amyisam-recover=BACKUP' /etc/my.cnf
		sed -i '$atmp_table_size=512M' /etc/my.cnf
		sed -i '$amax_connections=1500' /etc/my.cnf
		sed -i '$aquery_cache_limit=256M' /etc/my.cnf
		sed -i '$aquery_cache_size=1024M' /etc/my.cnf
		sed -i '$aexpire_logs_days=10' /etc/my.cnf
		sed -i '$amax_binlog_size=100M' /etc/my.cnf
		
		sed -i '$a[client]' /etc/my.cnf
		sed -i '$aport=3306' /etc/my.cnf
		sed -i '$adefault-character-set=utf8' /etc/my.cnf

		sed -i '$a[mysqld_safe]' /etc/my.cnf
		sed -i '$alog-error=/var/log/mysqld.log' /etc/my.cnf
		sed -i '$asocket=/var/lib/mysql/mysql.sock' /etc/my.cnf
		sed -i '$apid-file=/var/lib/mysql/mysql.pid' /etc/my.cnf
		sed -i '$anice=0' /etc/my.cnf

		sed -i '$a[mysqldump]' /etc/my.cnf
		sed -i '$aquick' /etc/my.cnf
		sed -i '$aquote-names' /etc/my.cnf
		sed -i '$amax_allowed_packet=1024M' /etc/my.cnf

		sed -i '$a[mysql]' /etc/my.cnf
		sed -i '$a[isamchk]' /etc/my.cnf
		sed -i '$akey_buffer=16M' /etc/my.cnf
...
OracleInstall(){
	...
	rpm -ivh --force --nodeps $all_packages_path/centos6.8_oracle11gPackages/part1/*.rpm 
	rpm -ivh --force --nodeps $all_packages_path/centos6.8_oracle11gPackages/part2_xhost/*.rpm 
	...
	if cat /etc/sysctl.conf | grep Oracle11gR2 >/dev/null
		...
		sed -i '$a# Oracle11gR2 kernel parameters' /etc/sysctl.conf
		...
		sed -i '$akernel.shmmax=2147483648' /etc/sysctl.conf
	...
	if cat /etc/security/limits.conf | grep Oracle11gR2 >/dev/null
		...
		sed -i '$a# Oracle11gR2 shell limits' /etc/security/limits.conf
		sed -i '$ayuantiaotech soft nproc 2048' /etc/security/limits.conf
		sed -i '$ayuantiaotechhard nproc 16384' /etc/security/limits.conf
		sed -i '$ayuantiaotechsoft nofile 1024' /etc/security/limits.conf
		sed -i '$ayuantiaotechhard nofile 65536' /etc/security/limits.conf
		sed -i '$ayuantiaotechsoft stack 10240' /etc/security/limits.conf
	fi
	...
	if cat /etc/pam.d/login | grep Oracle11gR2 >/dev/null
		...
		sed -i '$a# Oracle11gR2 shell limits' /etc/pam.d/login
	...
	if cat /etc/pam.d/su | grep Oracle11gR2 >/dev/null
		...
		sed -i '$a# Oracle11gR2 shell limits' /etc/pam.d/su
		sed -i '$asession required pam_limits.so' /etc/pam.d/su
	fi
	...
	if cat /etc/profile | grep ORACLE_HOME >/dev/null
		...
		sed -i '$a# Oracle11gR2 shell limits' /etc/profile
		sed -i '$aexport ORACLE_BASE=/opt/app/oracle' /etc/profile
		sed -i '$aexport ORACLE_HOME=$ORACLE_BASE/product/11.2.0/dbhome_1' /etc/profile
		sed -i '$aexport ORACLE_OWNER=yuantiaotech' /etc/profile
		sed -i '$aexport ORACLE_SID=orcl' /etc/profile
		sed -i '$aexport NLS_LANG=.AL32UTF8' /etc/profile
		sed -i '$aexport PATH=$PATH:$ORACLE_HOME/bin' /etc/profile
		# 生效配置
		source /etc/profile
	fi
	...
	if [ ! -f /etc/init.d/oracle ]; then
		echo '#! /bin/sh' >> /etc/init.d/oracle
		sed -i '$aexport ORACLE_BASE=/opt/app/oracle' /etc/init.d/oracle
		sed -i '$aexport ORACLE_HOME=/opt/app/oracle/product/11.2.0/dbhome_1' /etc/init.d/oracle
		sed -i '$aexport ORACLE_OWNER=yuantiaotech' /etc/init.d/oracle
		sed -i '$aexport ORACLE_SID=orcl' /etc/init.d/oracle
		sed -i '$aexport PATH=$PATH:$ORACLE_HOME/bin' /etc/init.d/oracle
		sed -i '$aif [ ! -f $ORACLE_HOME/bin/dbstart -o ! -d $ORACLE_HOME ]' /etc/init.d/oracle
		sed -i '$athen' /etc/init.d/oracle
		sed -i '$aecho "Oracle startup: cannot start"' /etc/init.d/oracle
		sed -i '$aexit 1' /etc/init.d/oracle
		sed -i '$afi' /etc/init.d/oracle
		sed -i '$acase "$1" in' /etc/init.d/oracle
		sed -i '$astart)' /etc/init.d/oracle
		sed -i '$a# Oracle listener and instance startup' /etc/init.d/oracle
		sed -i '$aecho -n "Starting Oracle: "' /etc/init.d/oracle
		sed -i '$asu - $ORACLE_OWNER -c "$ORACLE_HOME/bin/dbstart $ORACLE_HOME"' /etc/init.d/oracle
		sed -i '$aecho "OK"' /etc/init.d/oracle
		sed -i '$a;;' /etc/init.d/oracle
		sed -i '$astop)' /etc/init.d/oracle
		sed -i '$a# Oracle listener and instance shutdown' /etc/init.d/oracle
		sed -i '$aecho -n "Shutdown Oracle: "' /etc/init.d/oracle
		sed -i '$asu - $ORACLE_OWNER -c "$ORACLE_HOME/bin/dbshut $ORACLE_HOME"' /etc/init.d/oracle
		sed -i '$aecho "OK"' /etc/init.d/oracle
		sed -i '$a;; ' /etc/init.d/oracle
		sed -i '$areload|restart)' /etc/init.d/oracle
		sed -i '$a$0 stop' /etc/init.d/oracle
		sed -i '$a$0 start' /etc/init.d/oracle
		sed -i '$a;;' /etc/init.d/oracle
		sed -i '$a*)' /etc/init.d/oracle
		sed -i '$aecho "Usage: `basename $0` start|stop|restart|reload"' /etc/init.d/oracle
		sed -i '$aexit 1' /etc/init.d/oracle
		sed -i '$aesac' /etc/init.d/oracle
		sed -i '$aexit 0' /etc/init.d/oracle
...
EOF
	...
	if cat /etc/sysctl.conf | grep Oracle11gR2 >/dev/null
		then
		sed -i '/Oracle11gR2/d' /etc/sysctl.conf
	...
	if cat /etc/security/limits.conf | grep Oracle11gR2 >/dev/null
		then
		sed -i '/Oracle11gR2/d' /etc/security/limits.conf
		sed -i '/yuantiaotech soft nproc 2048/d' /etc/security/limits.conf
		sed -i '/yuantiaotechhard nproc 16384/d' /etc/security/limits.conf
		sed -i '/yuantiaotechsoft nofile 1024/d' /etc/security/limits.conf
		sed -i '/yuantiaotechhard nofile 65536/d' /etc/security/limits.conf
		sed -i '/yuantiaotechsoft stack 10240/d' /etc/security/limits.conf
	...
	if cat /etc/pam.d/su | grep Oracle11gR2 >/dev/null
		then
		sed -i '/Oracle11gR2/d' /etc/pam.d/su
		sed -i '/session required pam_limits.so/d' /etc/pam.d/su
	...
	if cat /etc/pam.d/login | grep Oracle11gR2 >/dev/null
		then
		sed -i '/Oracle11gR2/d' /etc/pam.d/login
	...
	if cat /etc/profile | grep Oracle11gR2 >/dev/null
		then
		sed -i '/Oracle11gR2/d' /etc/profile
		...
		sed -i '/export NLS_LANG=.AL32UTF8/d' /etc/profile
...
ActiveMQInstall(){
	...
	# ActiveMQ内存限制
	sed -i '2a ACTIVEMQ_OPTS_MEMORY=\"-Xms512m -Xmx1024m\"' $ActiveMQinstallPath/$ActiveMQFileName/bin/activemq
	
	# 配置脚本JAVA_HOME路径
	ln -s $ActiveMQinstallPath/$ActiveMQFileName/bin/activemq /etc/init.d/
	sed -i '2a export JAVA_HOME='$(echo $JAVA_HOME)'' /etc/init.d/activemq
	sed -i '2a ACTIVEMQ_HOME='$(echo $ActiveMQinstallPath)'/'$(echo $ActiveMQFileName)'' /etc/init.d/activemq
...
addHostName(){
	...
	then
		...
		sed -i '$a'$(echo $myIP)'    '$(echo $myHostName)'' /etc/hosts
	fi
}
